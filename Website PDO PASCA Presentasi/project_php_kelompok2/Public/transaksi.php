...
<body>
                            ...
                            <div class="row g-2 mb-4 align-items-end" style="background: #F8FAFC; padding: 20px; border-radius: 12px; border: 1px solid #CBD5E1;">
                                <div class="col-md-5">
                                    ...
                                    <select id="pilih_menu" class="form-select">
                                        <option value="" disabled selected>Cari menu...</option>
                                        <?php foreach($menus as $mn): ?>
                                            <option value="<?= $mn['Id_Menu'] ?>" 
                                                    data-nama="<?= htmlspecialchars($mn['Nama_Menu']) ?>" 
                                                    data-harga-reguler="<?= $mn['Harga_Reguler'] ?>" 
                                                    data-harga-large="<?= $mn['Harga_Large'] ?>">
                                                <?= htmlspecialchars($mn['Nama_Menu']) ?> - Rp <?= number_format($mn['Harga_Reguler'], 0, ',', '.') ?> (Reg) / Rp <?= number_format($mn['Harga_Large'], 0, ',', '.') ?> (Lrg)
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
    ...
    <script>
        function tambahKeKeranjang() {
            ...
            let ukuran = ukuranSelect.value;

            if (menu.value === "") {
                Swal.fire({ title: 'Menu Belum Dipilih!', icon: 'warning', confirmButtonColor: '#07779D' });
                return;
            }
            if (isNaN(qty) || qty <= 0) {
                Swal.fire({ title: 'Jumlah Tidak Valid!', icon: 'error', confirmButtonColor: '#07779D' });
                return;
            }

            let nama = menu.options[menu.selectedIndex].getAttribute('data-nama');
            let hargaReguler = parseFloat(menu.options[menu.selectedIndex].getAttribute('data-harga-reguler'));
            let hargaLarge = parseFloat(menu.options[menu.selectedIndex].getAttribute('data-harga-large'));
            
            let harga = (ukuran === 'Large') ? hargaLarge : hargaReguler;
            let tbody = document.getElementById('keranjang_body');
            
            // CEK APAKAH MENU DENGAN UKURAN YANG SAMA SUDAH ADA DI KERANJANG
            let existingRow = null;
            let rows = tbody.querySelectorAll('tr');
            rows.forEach(row => {
                if(row.getAttribute('data-id') === menu.value && row.getAttribute('data-ukuran') === ukuran) {
                    existingRow = row;
                }
            });

            if (existingRow) {
                // JIKA SUDAH ADA: Update jumlah (qty) dan subtotal di baris tersebut
                let existingQtyInput = existingRow.querySelector('input[name="qty[]"]');
                let currentQty = parseInt(existingQtyInput.value);
                let newQty = currentQty + qty;

                if (newQty > 99) {
                    Swal.fire({ title: 'Batas Maksimal!', text: 'Maksimal 99 item per menu.', icon: 'warning', confirmButtonColor: '#07779D' });
                    return;
                }

                // Update angka qty di layar dan di input hidden
                existingQtyInput.value = newQty;
                existingRow.querySelector('.display-qty').innerText = newQty;

                // Update angka subtotal di layar dan di attribute
                let newSubtotal = harga * newQty;
                let subtotalCell = existingRow.querySelector('.subtotal-item');
                subtotalCell.setAttribute('data-subtotal', newSubtotal);
                
                let formatNewSubtotal = new Intl.NumberFormat('id-ID', { minimumFractionDigits: 2 }).format(newSubtotal);
                subtotalCell.innerText = "Rp " + formatNewSubtotal;

            } else {
                // JIKA BELUM ADA: Bikin baris baru seperti biasa
                if (qty > 99) {
                    Swal.fire({ title: 'Batas Maksimal!', text: 'Maksimal 99 item.', icon: 'warning', confirmButtonColor: '#07779D' });
                    return;
                }
                
                let subtotal = harga * qty;
                let formatHarga = new Intl.NumberFormat('id-ID', { minimumFractionDigits: 2 }).format(harga);
                let formatSubtotal = new Intl.NumberFormat('id-ID', { minimumFractionDigits: 2 }).format(subtotal);

                let tr = document.createElement('tr');
                // Tambahkan attribute data-id dan data-ukuran untuk patokan pengecekan
                tr.setAttribute('data-id', menu.value);
                tr.setAttribute('data-ukuran', ukuran);
                
                tr.innerHTML = `
                    <td class="fw-semibold" style="color: #003F63;">
                        ${nama} <span class="badge bg-light text-dark border ms-1">${ukuran}</span>
                        <input type='hidden' name='id_menu[]' value='${menu.value}'>
                        <input type='hidden' name='ukuran[]' value='${ukuran}'>
                    </td>
                    <td>Rp ${formatHarga} <input type='hidden' name='harga[]' value='${harga}'></td>
                    <td class="text-center"><span class="display-qty">${qty}</span> <input type='hidden' name='qty[]' value='${qty}'></td>
                    <td class='text-end fw-semibold subtotal-item' style="color: #07779D;" data-subtotal='${subtotal}'>Rp ${formatSubtotal}</td>
            ...
            // Reset state inputan kasir
            menu.value = "";
            qtyInput.value = "1";
            ukuranSelect.value = "Reguler";
        }

        function hitungTotal() {
            let items = document.querySelectorAll('.subtotal-item');
            let total_harga = 0;
            items.forEach(item => { total_harga += parseFloat(item.getAttribute('data-subtotal')); });

            let pajak = total_harga * 0.10; 
            let grand_total = total_harga + pajak;

            let formatter = new Intl.NumberFormat('id-ID', { minimumFractionDigits: 2 });
            document.getElementById('display_total').innerText = "Rp " + formatter.format(total_harga);
            document.getElementById('display_pajak').innerText = "Rp " + formatter.format(pajak);
            document.getElementById('display_grand').innerText = "Rp " + formatter.format(grand_total);
        }
